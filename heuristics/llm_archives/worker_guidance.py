'''
code length: 1676
reason=height_oob util=0.6845 n_boxes=25
worker feedback：maximize space util
'''
            ...
            for x in range(heur.X - dx + 1):
                for y in range(heur.Y - dy + 1):
                    ...
                    if heur.feasibility.is_feasible(pallet, x, y, dx, dy, dz, z):
                        if box_volume > best_volume:
                            best_volume = box_volume
                            best_action = (slot_i, rot_id, x, y)
    
    return best_action

class WorkerGuidance(BaseHeuristic):
    name = "worker_guidance"
